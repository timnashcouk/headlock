 ...
 * Text Domain:     tn-security-headers
 * Domain Path:     /languages
 * Version:         0.3.0
 *
 * @package         Tn_Security_Headers
 */
...
function tn_enabled_security_headers(){
	...
	$enabled_security_headers = apply_filters( 'tn_enabled_security_headers', $enabled_security_headers );
	...
	if( is_array($enabled_security_headers ) ){
		foreach( $enabled_security_headers as $security_header ){
			...
			$function_call = 'tn_'.$security_header;
			if( function_exists( $function_call ) ){
				...
				// Generic Hook
				do_action( 'tn_security_headers', $security_header, $header );
			}
		}
	}
}

add_action( 'send_headers', 'tn_enabled_security_headers' );

/**
 * Sets the HSTS Security Header
 * Default Args: max-age 1 year, includesubdomains true, preload true.
 * Filter: tn_strict_transport_security
 * 
 ...
 * @return string
 */
function tn_strict_transport_security() {
	...
	$strict_transport_security_args = apply_filters( 'tn_strict_transport_security', $strict_transport_security_args );
 ...
 * 
 * Filter: tn_x_frame_options
 * 	Alternative to SAMEORIGIN
 * 		DENY Prevents iFrame of site within itself.
 * 
 * @return string
 */
function tn_x_frame_options() {
	// Set Default as sameorigin
	$frame_option = 'sameorigin';

	// Filter arguments for development and testing
	$frame_option = apply_filters( 'tn_x_frame_options', $frame_option );
 ...
 * Filter: tn_x_xss_protection
 * 
 *
 * @return string
 */
function tn_x_xss_protection() {
	// Standard Defaults for production
	...
	// Filter arguments for development and testing
	$xss_protection_args = apply_filters( 'tn_x_xss_protection', $xss_protection_args );
 ...
 * 
 * Filter: tn_x_content_type_options
 * 
 * @return string
 */
function tn_x_content_type_options() {
	// Set Default as nosniff
	$content_type_option = 'nosniff';

	// Filter arguments for development and testing
	$content_type_option = apply_filters( 'tn_x_content_type_options', $content_type_option );
 ...
 * Filter: tn_referrer_policy to adjust default
 * 
 * @return string
 */
function tn_referrer_policy() {

	// Set Default as strict-origin-when-cross-origin
	$referrer_policy = 'strict-origin-when-cross-origin';
	// Filter arguments for development and testing
	$referrer_policy = apply_filters( 'tn_referrer_policy', $referrer_policy );
...
function tn_content_security_policy() {
	/*
	...
	$content_security_policy_args = apply_filters( 'tn_content_security_policy', $content_security_policy_args );
	...
	$content_security_policy_sources = apply_filters( 'tn_content_security_policy_sources', $content_security_policy_sources );
